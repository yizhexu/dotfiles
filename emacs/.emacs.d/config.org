...
#+begin_src emacs-lisp
  (setq user-full-name "Yizhe Xu"
        ...
        calendar-latitude 39.670397
        calendar-longitude -104.984643)
#+end_src
...
(when (is-mac-p) (yizhe/append-to-path "/usr/local/anaconda3/bin"))
#+END_SRC
...
(when (is-mac-p)(setenv "WORKON_HOME" "/usr/local/anaconda3/envs"))
...
(use-package tramp
  ...
  (add-to-list 'tramp-remote-path "/home/yizhx/bin")
  (add-to-list 'tramp-remote-path "/home/josber/.nvm/versions/node/v11.4.0/bin")
  (add-to-list 'tramp-remote-path 'tramp-own-remote-path)

  ;; I don't like my history littering many spots, and don't always have a home directory on remotes.
  (setq tramp-histfile-override "/tmp/.yizhx_tramp_history")

  ;; Shamelessly stolen from amazon common.  Basically, when we go
  ;; through the bastions the message saying can not chdir to directory
  ;; will appear.  We are simply ensuring that the error start with "scp."
...
#+BEGIN_SRC emacs-lisp
(when (is-mac-p)
  (setq keybase-ref "/Volumes/Keybase\ (p2762265)/private/yizhe/references/"))
...
(use-package org-ref
  :init
  ;; setup org-ref
  (setq org-ref-default-bibliography (concat keybase-ref "library.bib")
