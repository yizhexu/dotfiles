...
#+RESULTS:
: ((gnu . http://elpa.gnu.org/packages/) (melpa . http://melpa.org/packages/) (melpa-stable . http://stable.melpa.org/packages/) (org . https://orgmode.org/elpa/))
...
#+RESULTS:
| org-index-file | (org-file-path projects.org) | (org-file-path tickler.org) |
...
(setq org-capture-templates '(("t" "Todo [inbox]" entry
                               ...
                               "* TODO %i%?")
                              ("T" "Tickler" entry
                               (file+headline (org-file-path "tickler.org") "Tickler")
                               "* %i%? \n %U")))
#+end_src

#+RESULTS:
| t | Todo [inbox] | entry | (file+headline org-index-file Tasks)                | * TODO %i%? |
| T | Tickler      | entry | (file+headline (org-file-path tickler.org) Tickler) | * %i%?      |

*** Refile Templates

#+begin_src emacs-lisp
    (setq org-refile-targets '(((org-file-path "projects.org") :maxlevel . 3)
                               ((org-file-path "later.org") :level . 1)
                               ((org-file-path "tickler.org") :maxlevel . 2)))
#+end_src

#+RESULTS:
: (((org-file-path projects.org) :maxlevel . 3) ((org-file-path later.org) :level . 1) ((org-file-path tickler.org) :maxlevel . 2))
